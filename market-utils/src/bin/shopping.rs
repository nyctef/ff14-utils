...
async fn main() {
        ...
        if let Ok(entry) = line {
            ...
            if parts.len() == 2 {
                if let Ok(quantity) = parts[0].parse::<u32>() {
                    shopping_list.insert(parts[1].to_string(), quantity);
                }
            }
        }
    }
    ...
    for (name, quantity) in &shopping_list {
        if let Some(item) = item_lookup.item_by_name_opt(name) {
            resolved_items.insert(item.id, *quantity);
        } else {
            eprintln!("Warning: Item '{}' not found in lookup", name);
        }
    }

    let item_ids: Vec<ItemId> = resolved_items.keys().cloned().collect();
    ...
    for (item_id, quantity) in &resolved_items {
        ...
        rows.push((name, quantity, formatted_cost));
    }
    ...
    for (name, quantity, formatted_cost) in rows {
        println!(
            "{:<width_name$}  {:>width_quantity$}  {:>width_cost$}",
            name,
            quantity,
